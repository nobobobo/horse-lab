...
from horse_lab.stacking import search_convex_blend_from_csv
...
def test_search_convex_blend_uses_latest_fold_as_holdout(tmp_path):
    meta_features_path = tmp_path / "meta_features.csv"
    artifact_dir = tmp_path / "blend"
    ...
    assert result.model_path == artifact_dir / "blend_model.json"
    assert result.predictions_path == artifact_dir / "blend_predictions.csv"
    ...
    assert result.report["weights"] == [
        {"feature_column": "pred__form__v1", "weight": 1.0},
        {"feature_column": "pred__market__v1", "weight": 0.0},
    ]
    ...
    assert model_payload["weights"] == [1.0, 0.0]


def test_stacking_search_blend_cli_writes_artifacts(tmp_path, capsys):
