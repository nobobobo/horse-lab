...
from horse_lab.stacking import run_residual_overlay_study_from_csv
...
def _write_meta_features(path: Path) -> None:
    _write(
        path,
        """
race_id,runner_id,target,label,fold_id,validation_start,validation_end,as_of,feature_version,pred__market_implied_probability__v1,pred__lightgbm_no_market__v1
...
def test_residual_overlay_study_writes_walk_forward_artifacts(tmp_path):
    meta_features_path = tmp_path / "meta_features.csv"
    artifact_dir = tmp_path / "residual_overlay"
    ...
    assert result.report_path == artifact_dir / "residual_overlay_report.json"
    assert result.predictions_path == artifact_dir / "residual_overlay_predictions.csv"
    assert (
        result.report["overlay_column"]
        == "pred__lightgbm_no_market__v1"
    )
    assert result.report["counts"]["evaluated_folds"] == 2
    assert result.report["overall"]["residual_overlay"]["observations"] == 4

    with result.predictions_path.open(encoding="utf-8") as handle:
        rows = tuple(csv.DictReader(handle))
    assert len(rows) == 4
    assert {row["method"] for row in rows} == {"residual_overlay"}
...
def test_residual_overlay_study_cli(tmp_path, capsys):
